...
foreach ($array_status as $idStatus)
{
    ...
    $sqlProjeto = "SELECT idProjeto, nomeProjeto, protocolo, pf.nome, pf.cpf, razaoSocial, cnpj, areaAtuacao, pfc.nome AS comissao, etapaProjeto, pro.idStatus AS idStatus 
                    FROM projeto AS pro
                    ...
                    INNER JOIN etapa_projeto AS st ON pro.idStatus = st.idStatus
                    WHERE pro.publicado = 1 AND pro.idStatus = i$idStatusdStatus" .$parecerista." ORDER BY idProjeto DESC";
    ...
    foreach ($queryStatus as $status)
    {
        ...
        <div class='form-group'>
            ...
            <form method="POST" action="<?= ($pf['idNivelAcesso'] == 2) ? "?perfil=smc_pesquisa_geral_resultado" : "?perfil=comissao_pesquisa_resultado"?>" class="form-horizontal" role="form">
                <button type="submit" class="label label-warning" name="idStatus" value="<?=$stidStatusStatus']?>">
                    <span>Total: <?=$num?></span>
                </button>
            </form>
        </div>
