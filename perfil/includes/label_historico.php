<div role="tabpanel" class="tab-pane fade" id="historico">
    ...
    <table class='table table-condensed'>
        <thead>
            ...
            <?php
            $sql_lista_historico = "SELECT st.status, hs.data AS data FROM historico_status AS hs INNER JOIN etapa_projeto AS st ON hs.idEtapaProjeto = st.idEtapaProjeto WHERE hs.idProjeto = '$idProjeto' ORDER BY hs.data,hs.idEtapaProjeto";
...
<br/>
  ...
  <?php
    $sql_lista_historico = "SELECT st.status AS h_st, rn.data AS dt, rn.dataPublicacao, rn.linkPublicacao, us.nome AS user FROM historico_publicacao AS rn INNER JOIN pessoa_fisica AS us ON rn.idUsuario = us.idPf INNER JOIN etapa_projeto AS st ON rn.idEtapaProjeto = st.idEtapaProjeto WHERE rn.idProjeto = '$idProjeto' ORDER BY rn.data,rn.idEtapaProjeto";
            $query_lista_historico = mysqli_query($con,$sql_lista_historico);
            $num = mysqli_num_rows($query_lista_historico);
                if($num > 0)
                    {
