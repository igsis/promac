<?php
	...
	{	
		$email = $_POST['email'];
		$sql = "SELECT email FROM incentivador_pessoa_fisica WHERE email = '$email'";
		...
		if($num > 0)
		{
			$etapa = 2;
			$tentativa = 0;
			$pf = recuperaDados("incentivador_pessoa_fisica","email",$email);
		}
		else
		{
	...
	{	
		...
		$sql = "SELECT email, idFraseSeguranca, respostaFrase FROM incentivador_pessoa_fisica WHERE email = '$email' AND idFraseSeguranca = '$frase' AND respostaFrase = '$resposta'";
		$query = mysqli_query($con,$sql);
		$num = mysqli_num_rows($query);
		if($num > 0)
		{
	...
	if (isset($_POST['enviarSenha']))
	{
		...
		if(($_POST['senha01'] != "") AND (strlen($_POST['senha01']) >= 5))
		{
			if($_POST['senha01'] == $_POST['senha02'])
			{
				...
				$sql_senha = "UPDATE `incentivador_pessoa_fisica` SET `senha` = '$senha01' WHERE `idPf` = '$idUsuario';";
				...
				if($query_senha)
				{
					...
					echo "<script type=\"text/javascript\">
						  window.setTimeout(\"location.href='login_incentivador_pf.php';\", 3000);
						</script>";
				}
				else
				{
					...
					echo "<script type=\"text/javascript\">
						  window.setTimeout(\"location.href='recuperar_senha_incentiv_pf.php';\", 3000);
						</script>";
				}
			}
			else
