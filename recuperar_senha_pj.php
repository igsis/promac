<?php
	...
	{	
		$email = $_POST['email'];
		$sql = "SELECT email FROM pessoa_juridica WHERE email = '$email'";
		...
		if($num > 0)
		{
			$etapa = 2;
			$tentativa = 0;
			$pj = recuperaDados("pessoa_juridica","email",$email);
		}
		else
		{
	...
	{	
		...
		$sql = "SELECT email, idFraseSeguranca, respostaFrase FROM pessoa_juridica WHERE email = '$email' AND idFraseSeguranca = '$frase' AND respostaFrase = '$resposta'";
		$query = mysqli_query($con,$sql);
		$num = mysqli_num_rows($query);
		if($num > 0)
		{
	...
	if (isset($_POST['enviarSenha']))
	{
		...
		if(($_POST['senha01'] != "") AND (strlen($_POST['senha01']) >= 5))
		{
			if($_POST['senha01'] == $_POST['senha02'])
			{
				...
				$sql_senha = "UPDATE `pessoa_juridica` SET `senha` = '$senha01' WHERE `idPj` = '$idUsuario';";
				...
				if($query_senha)
				{
					...
					echo "<script type=\"text/javascript\">
						  window.setTimeout(\"location.href='login_pj.php';\", 3000);
						</script>";
				}
				else
				{
					...
					echo "<script type=\"text/javascript\">
						  window.setTimeout(\"location.href='recuperar_senha_pj.php';\", 3000);
						</script>";
				}
			}
			else
	...
	<div class="container">
		<div class="row">
			...
			<?php	}
				...
				<!-- Solicitando E-mail -->
				<form method="POST" action="./recuperar_senha_pj.php">
